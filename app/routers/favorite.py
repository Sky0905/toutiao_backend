...
async def create_favorite(payload: FavoriteCreate, db: AsyncSession = Depends(db_session)):
    ...
    try:
        return await favorite_crud.create_favorite(db, payload)
    except IntegrityError as exc:
        await db.rollback()
        raise HTTPException(status_code=409, detail="Favorite already exists or resource not found") from exc
...
async def read_favorites(user_id: int, db: AsyncSession = Depends(db_session)):
    # 路径参数 user_id 必须是整数。
    return await favorite_crud.list_favorites(db, user_id)
